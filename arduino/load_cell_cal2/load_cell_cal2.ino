...
void setup() {
  ...
  scale.set_scale();
  scale.tare(64);	//Reset the scale to 0, takes average of readings over 80 bytes to get zero
...
void loop() {
  static unsigned long startTime = millis();// set timer

  Serial.print(scale.get_units(128), 5); //number inside get_units function() is bytes used for average scale reading
  unsigned long timeStamp = millis() - startTime; // print time stamp at this measurement
  Serial.print(", ");
